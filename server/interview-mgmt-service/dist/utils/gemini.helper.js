...
class GeminiHelper {
    static async generateAptitudeQuestions() {
        try {
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${"AIzaSyCMRTsmwTxGgizrCRGKuwNclvftHuVsr2g"}`, {
                contents: [
                    {
                        parts: [
                            {
    ...
    static async evaluateCode(files) {
        try {
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyCMRTsmwTxGgizrCRGKuwNclvftHuVsr2g`, {
                contents: [
                    {
            ...
            return JSON.parse(generatedText.replace("```json\n", "").replace("\n```", ""));
        }
        catch (error) {
            console.error("Gemini API error:", error);
